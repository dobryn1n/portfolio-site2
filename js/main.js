$(function(){
	$('.block-popup span').click(function(){
		$('.block-popup, .overlay').fadeOut();
	})
	setTimeout(function(){
		$('.block-popup, .overlay').fadeIn();
	},2000)

	$('.header__slider').slick({
		infinite: true,
		fade: true,
		prevArrow: '<img class="slider-arrow slider-arrow_left" src="img/backsurf.svg" alt="">',
		nextArrow: '<img class="slider-arrow slider-arrow_right" src="img/gotosurf.svg" alt="">',
		asNavFor: '.slider-bottom'
	});

	$('.slider-bottom').slick({
		slidesToShow: 4,
		slidesToScroll: 1,
		arrows: false,
		asNavFor: '.header__slider'

	});

	$('.surf-slider').slick({
		slidesToShow: 4,
		slidesToScroll: 1,
		prevArrow: '<img class="slider-arrow slider-arrow_left" src="img/backsurf.svg" alt="">',
		nextArrow: '<img class="slider-arrow slider-arrow_right" src="img/gotosurf.svg" alt="">',
		asNavFor: '.slider-map'
	});
	$('.slider-map').slick({
		slidesToShow: 8,
		slidesToScroll: 1,
		asNavFor: '.surf-slider',
		arrows: false,
		focusOnSelect: true
	});

	$('.travel-slider').slick({
		infinite: true,
		fade: true,
		prevArrow: '<img class="slider-arrow slider-arrow_left" src="img/backsurf.svg" alt="">',
		nextArrow: '<img class="slider-arrow slider-arrow_right" src="img/gotosurf.svg" alt="">'
	});
	$('.sleep-slider, .shop-slider').slick({
		infinite: true,
		fade: true,
		prevArrow: '<img class="slider-arrow slider-arrow_left" src="img/backsurf.svg" alt="">',
		nextArrow: '<img class="slider-arrow slider-arrow_right" src="img/gotosurf.svg" alt="">'
	});




	$('<div class="quantity-nav"><div class="quantity-button quantity-up"><img src="img/plus.png" alt=""></div><div class="quantity-button quantity-down"><img src="img/minus.png" alt=""></div></div>').insertAfter('.quantity input');
	$('.quantity').each(function() {
		var spinner = $(this),
		input = spinner.find('input[type="number"]'),
		btnUp = spinner.find('.quantity-up'),
		btnDown = spinner.find('.quantity-down'),
		min = input.attr('min'),
		max = input.attr('max');

		btnUp.click(function() {
			var oldValue = parseFloat(input.val());
			if (oldValue >= max) {
				var newVal = oldValue;
			} else {
				var newVal = oldValue + 1;
			}
			spinner.find("input").val(newVal);
			spinner.find("input").trigger("change");
		});

		btnDown.click(function() {
			var oldValue = parseFloat(input.val());
			if (oldValue <= min) {
				var newVal = oldValue;
			} else {
				var newVal = oldValue - 1;
			}
			spinner.find("input").val(newVal);
			spinner.find("input").trigger("change");
		});

	});
	  $('.quantity-button').on('click', function(){
    let summ = $('.nights').val() * $('.summ').data('nights') + ($('.guests').val() -1) * $('.summ').data('guests');
  $('.summ').html('$' + summ);
  });

  let summ = $('.nights').val() * $('.summ').data('nights') + ($('.guests').val() -1) * $('.summ').data('guests');
  $('.summ').html('$' + summ);

  $('.shop-slider__wrapper-info__skate-box__circle').on('click', function (){
  	$(this).toggleClass('active')
  });
});
